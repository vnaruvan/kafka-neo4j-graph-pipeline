...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # TODO: Your code here
        csvfile = os.path.basename(save_loc)
        urlcsv = f"file:///{csvfile}"
        neo4j_save_loc = f"/var/lib/neo4j/import/{csvfile}"
        with self.driver.session() as projneo4jsession:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] clearing graph")
            projneo4jsession.run("MATCH (n) DETACH DELETE n")
            print(f"[{datetime.now().strftime('%H:%M:%S')}] loading from {urlcsv}")
            query_text = """
            LOAD CSV WITH HEADERS FROM $csvpath AS rec
            WITH rec
            WHERE rec.PULocationID IS NOT NULL AND rec.DOLocationID IS NOT NULL
            MERGE (pustartloc:Location {name: toInteger(rec.PULocationID)})
            MERGE (doendloc:Location   {name: toInteger(rec.DOLocationID)})
            CREATE (pustartloc)-[:TRIP {distance: toFloat(rec.trip_distance), fare: toFloat(rec.fare_amount), pickup_dt: rec.tpep_pickup_datetime, dropoff_dt: rec.tpep_dropoff_datetime}]->(doendloc)"""
            projneo4jsession.run(query_text, {"csvpath": urlcsv})
            print(f"[{datetime.now().strftime('%H:%M:%S')}] data loaded successfully")
...
def main():
    ...
    while attempt < total_attempts:
        try:
            ...
            data_loader = DataLoader(uri, user, pwd)
